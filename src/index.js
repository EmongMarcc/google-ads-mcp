...
function getCustomer(customerId) {
  ...
  const id = (customerId || GOOGLE_CUSTOMER_ID).replace(/-/g, "");
  return client.Customer({
    customer_id: id,
    refresh_token: GOOGLE_ADS_REFRESH_TOKEN,
    login_customer_id: GOOGLE_MANAGER_ACCOUNT_ID
      ? GOOGLE_MANAGER_ACCOUNT_ID.replace(/-/g, "")
      : undefined,
  });
}
...
function createServer() {
  ...
  server.tool(
    ...
    {
      customer_id: z.string().optional().describe("Google Ads customer ID (overrides default)"),
  ...
  server.tool(
    ...
    async ({ customer_id, date_from, date_to, metric, limit, order }) => {
      ...
      const results = await customer.query(`
        ...
        ORDER BY ${metricMap[metric]} ${orderDir}
        LIMIT ${limit}
      `);
      return {
        content: [{ type: "text", text: JSON.stringify(results, null, 2) }],
  ...
  server.tool(
    ...
    async ({ customer_id, campaign_id, status }) => {
      ...
      const result = await customer.campaigns.update([
        {
          resource_name: `customers/${(customer_id || GOOGLE_CUSTOMER_ID).replace(/-/g, "")}/campaigns/${campaign_id}`,
          status,
        },
      ]);
      return {
  ...
  server.tool(
    ...
    async ({ customer_id, budget_id, amount_usd }) => {
      ...
      const result = await customer.campaignBudgets.update([
        {
          resource_name: `customers/${(customer_id || GOOGLE_CUSTOMER_ID).replace(/-/g, "")}/campaignBudgets/${budget_id}`,
          amount_micros,
        },
      ]);
      return {
  ...
  server.tool(
    ...
    "Get keyword ideas and search volume estimates using Keyword Planner",
    {
      ...
      language_id: z.string().optional().default("1000").describe("Language ID (1000 = English)"),
    ...
    async ({ customer_id, keywords, language_id, geo_target_id }) => {
      const client = getClient();
      const id = (customer_id || GOOGLE_CUSTOMER_ID).replace(/-/g, "");
      const loginId = GOOGLE_MANAGER_ACCOUNT_ID
        ? GOOGLE_MANAGER_ACCOUNT_ID.replace(/-/g, "")
        : undefined;
      ...
      const geoTargets = geo_target_id
        ? [`geoTargetConstants/${geo_target_id}`]
        : [];
  ...
  server.tool(
    ...
    "Get quality scores for all keywords — identify low quality keywords",
    {
      customer_id: z.string().optional(),
      campaign_id: z.string().optional(),
      min_score: z.number().optional().default(0).describe("Filter keywords with score below this"),
    },
...
const httpServer = http.createServer(async (req, res) => {
  if (req.method === "GET" && req.url === "/health") {
    ...
    res.end(JSON.stringify({ status: "ok", service: "google-ads-mcp" }));
    return;
  }
